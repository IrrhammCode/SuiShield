...
const MOCK_RESPONSES: Record<string, { content: string; metadata: MessageMetadata }> = {
  default: {
    content: `I've analyzed your query using Tatum's Data API and historical data from Walrus.

Here's what I found based on available blockchain data:

**Network Overview (Sui Mainnet)**
- Current TPS: ~4,200 transactions/second  
- 24h Transaction Count: 362M+
- Active Wallets (24h): 184,000+
- Average Gas Fee: 0.0015 SUI

The data above is pulled in real-time via Tatum RPC. For deeper historical analysis going back further than 3 days, I reference the Walrus dataset stored as a verified blob on the Walrus network.

Is there a specific wallet address, time range, or protocol you'd like me to dig deeper into?`,
    metadata: {
      sources: [
        { type: "tatum-rpc", label: "Tatum Sui RPC", endpoint: "sui-mainnet.gateway.tatum.io", chain: "Sui" },
        { type: "walrus", label: "Sui Transactions Dataset", blobId: "BLOB_ID_SUI_TXS", chain: "Sui" },
      ],
      toolsUsed: ["gateway_execute_rpc", "get_transaction_history"],
      executionTime: 1842,
    },
  },
  wallet: {
    content: `**Wallet Security Analysis Complete**

I've run a comprehensive security check on this address using Tatum's \`check_malicious_address\` tool and cross-referenced with 6 months of historical data from Walrus.

**Risk Assessment: ⚠️ MEDIUM RISK**
- Malicious flag: Not directly flagged
- Connected to 3 flagged addresses (2nd degree)
- Unusual transaction pattern detected in March 2025
- High-frequency micro-transactions: possible wash trading

**Transaction Summary (6 months via Walrus dataset)**
- Total transactions: 1,247
- Unique counterparties: 89
- Largest single transfer: 45,200 SUI
- Average daily activity: 6.9 txns/day

**Recommendation:** Exercise caution. While not directly flagged, the wallet has notable connections to flagged addresses. Verify the counterparty independently before large transfers.`,
    metadata: {
      sources: [
        { type: "tatum-mcp", label: "check_malicious_address", chain: "Sui" },
        { type: "walrus", label: "Sui Transactions Dataset", blobId: "BLOB_ID_SUI_TXS", chain: "Sui" },
        { type: "tatum-api", label: "Tatum Data API", chain: "Sui" },
      ],
      toolsUsed: ["check_malicious_address", "get_transaction_history", "get_wallet_portfolio"],
      executionTime: 2341,
      riskScore: 45,
      walletInfo: {
        address: "0x742d35Cc6634C0532925a3b8D4C9C2b97e4C3981",
        chain: "Sui",
        riskScore: 45,
        riskLevel: "medium",
        isMalicious: false,
        labels: ["Active Trader", "DeFi User"],
  ...
  price: {
    content: `**Price History Analysis — SUI/USD**

Retrieved from Walrus blob: 4-year OHLCV dataset (1-minute candles)

**Performance Summary:**
- Jan 2026: $3.21 → $4.87 (+51.7%)
- Feb 2026: $4.87 → $5.93 (+21.8%)
- Mar 2026: $5.93 → $4.12 (-30.5%)
- Apr 2026: $4.12 → $6.44 (+56.3%)
- May 2026: $6.44 → Current: $6.91 (+7.3%)

**Key Observations:**
- Strong uptrend since Q1 2026 despite March correction
- Volume surged 340% during April recovery — institutional accumulation pattern
- Sui ecosystem growth correlating with price: +180% DeFi TVL since Jan 2026

*Data sourced directly from Walrus blob — verified, immutable, decentralized.*`,
    metadata: {
      sources: [
        { type: "walrus", label: "Crypto Price OHLCV 4yr Dataset", blobId: "BLOB_ID_PRICE_OHLCV", chain: "Multi-Chain" },
        { type: "tatum-api", label: "Tatum Exchange Rates API", chain: "Multi-Chain" },
      ],
      toolsUsed: ["get_exchange_rate", "get_block_by_time"],
      executionTime: 987,
      charts: [
        {
          type: "area",
          title: "SUI Price History (2026)",
          xKey: "month",
          yKey: "price",
          color: "#4F37FD",
          data: [
            { month: "Jan", price: 3.21 },
            { month: "Feb", price: 4.87 },
            { month: "Mar", price: 5.93 },
            { month: "Apr (dip)", price: 4.12 },
            { month: "Apr (rec)", price: 6.44 },
            { month: "May", price: 6.91 },
          ],
        },
      ],
    },
...
function detectQueryType(query: string): string {
  ...
  if (lower.includes("wallet") || lower.includes("address") || lower.includes("0x") || lower.includes("safe") || lower.includes("scam") || lower.includes("malicious")) return "wallet";
  if (lower.includes("price") || lower.includes("sui") || lower.includes("btc") || lower.includes("eth") || lower.includes("history") || lower.includes("candle")) return "price";
  return "default";
}
...
What would you like to know?`,
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: "var(--bg-base)" }}>
      {/* Top bar */}
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/5 glass-bright z-10">
