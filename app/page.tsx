...
import Floating3DCard, { FloatingOrb, TiltText } from "@/components/Floating3D";
import ScrollReveal, { Stagger, CountUp, MagneticButton } from "@/components/ScrollAnimations";
    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? "glass-bright py-3 shadow-[0_8px_40px_rgba(0,0,0,0.5)]" : "py-5"}`}>
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link href="/trust-graph" className="text-[#525880] hover:text-white text-sm transition-colors hidden sm:block">Trust Graph</Link>
          <Link href="/explore" className="text-[#525880] hover:text-white text-sm transition-colors hidden sm:block">Explore</Link>
          <Link href="/chat" className="text-[#525880] hover:text-white text-sm transition-colors hidden sm:block">Chat</Link>
          <DualWalletButton />
        </div>
      </div>
    </nav>
...
function FeatureCard({ icon, title, description, accent }: { icon: React.ReactNode; title: string; description: string; accent: "primary" | "secondary" }) {
  return (
    <Floating3DCard
      ...
      glowColor={accent === "primary" ? "rgba(0,229,255,0.15)" : "rgba(255,0,122,0.15)"}
    >
...
function LiveDemoCard() {
  return (
    <Floating3DCard className="max-w-lg mx-auto" glowColor="rgba(0,229,255,0.2)">
...
export default function LandingPage() {
  return (
    <div className="relative min-h-screen overflow-hidden" style={{ background: "var(--bg-base)" }}>
      ...
      <ParticleCanvas
        particleCount={60}
        colors={["#00E5FF", "#FF007A", "#66F5FF"]}
        connectDistance={120}
        mouseRadius={180}
      />

      {/* Floating Orbs */}
      <FloatingOrb size={400} color="#00E5FF" className="top-[10%] -left-[10%]" />
      <FloatingOrb size={300} color="#FF007A" className="top-[30%] -right-[8%]" />
      <FloatingOrb size={250} color="#66F5FF" className="bottom-[20%] left-[10%]" />
          ...
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-8" staggerDelay={150}>
            <div className="text-center">
              <div className="font-display font-bold text-3xl md:text-4xl text-gradient-primary mb-1">
                <CountUp end={11} suffix="TB+" />
              </div>
              <div className="text-[#525880] text-xs uppercase tracking-wider font-medium">Blockchain Data</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl md:text-4xl text-gradient-primary mb-1">
                <CountUp end={4} />
              </div>
              <div className="text-[#525880] text-xs uppercase tracking-wider font-medium">Chains Supported</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl md:text-4xl text-gradient-primary mb-1">
                <CountUp end={59} />
              </div>
              <div className="text-[#525880] text-xs uppercase tracking-wider font-medium">AI Tools</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl md:text-4xl text-gradient-primary mb-1">∞</div>
              <div className="text-[#525880] text-xs uppercase tracking-wider font-medium">Proof Storage</div>
            </div>
          </Stagger>
        </div>
      </section>
        ...
        <Stagger className="grid md:grid-cols-2 gap-4" staggerDelay={100}>
          {[
            ...
            <div key={title} className="flex items-start gap-4 p-5 rounded-2xl border border-red-500/8 bg-red-500/3 backdrop-blur-sm">
              <div className="w-10 h-10 rounded-xl bg-red-500/8 flex items-center justify-center flex-shrink-0">{icon}</div>
              <div>
      ...
      <section className="relative max-w-5xl mx-auto px-6 py-24 z-10">
        ...
        <Stagger className="grid md:grid-cols-2 gap-4" staggerDelay={100}>
          <FeatureCard icon={<Zap className="w-5 h-5" />} title="Real-time via Tatum Sui RPC" description="Balance, objects, transactions — live from Sui mainnet through Tatum's enterprise-grade infrastructure." accent="primary" />
      ...
      <section className="relative max-w-5xl mx-auto px-6 py-24 z-10">
        ...
        <Stagger className="grid md:grid-cols-2 gap-4" staggerDelay={100}>
          {[
            { icon: <TrendingUp className="w-5 h-5" />, title: "DeFi Farmers", question: "Is this yield safe?", flow: "Paste pool address → Agent checks TVL trend, sustainability, concentration risk → Trust score on-chain", color: "cyan" },
            { icon: <Globe className="w-5 h-5" />, title: "NFT Buyers", question: "Is this creator legit?", flow: "Paste creator address → Agent checks wallet age, past projects, wash trading → Verdict with evidence", color: "magenta" },
            { icon: <Users className="w-5 h-5" />, title: "P2P Traders", question: "Is this wallet clean?", flow: "Paste counterparty → Agent checks money flow, scam database, network risk → Instant verdict", color: "blue" },
            { icon: <Shield className="w-5 h-5" />, title: "New Users", question: "Where do I start?", flow: "Paste any address → Simple safe/not verdict → Community-verified reports protect you", color: "green" },
          ].map(({ icon, title, question, flow, color }) => (
            <Floating3DCard key={title} className="card-premium p-6 cursor-pointer" glowColor={`rgba(0,229,255,0.1)`}>
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className={`text-sm font-medium mt-0.5 ${
                      color === "cyan" ? "text-cyan-400" : color === "magenta" ? "text-magenta-400" : color === "blue" ? "text-blue-400" : "text-green-400"
      ...
      <footer className="relative max-w-5xl mx-auto px-6 py-12 z-10 border-t border-white/5">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 text-xs text-[#525880]">
            <a href="https://tatum.io" target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors flex items-center gap-1">Tatum <ExternalLink className="w-3 h-3" /></a>
            <a href="https://walrus.space" target="_blank" rel="noopener noreferrer" className="hover:text-magenta-400 transition-colors flex items-center gap-1">Walrus <ExternalLink className="w-3 h-3" /></a>
            <a href="https://sui.io" target="_blank" rel="noopener noreferrer" className="hover:text-blue-400 transition-colors flex items-center gap-1">Sui <ExternalLink className="w-3 h-3" /></a>
          </div>
        </div>
      </footer>
    </div>
