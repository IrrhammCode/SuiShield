...
// ─── Chain icon mapping — official logos as <img> ──────────
...
function getChainIcon(iconName: string, color: string): React.ReactNode {
  const logo = CHAIN_LOGOS[iconName];
  if (logo) {
    return <img src={logo} alt={iconName} className="w-6 h-6 object-contain" />;
  }
  // Fallback to Lucide icons for non-chain icons
  const lucideMap: Record<string, React.ReactNode> = {
    chart: <BarChart2 className="w-6 h-6" style={{ color }} />,
    zap: <Zap className="w-6 h-6" style={{ color }} />,
    bridge: <Link2 className="w-6 h-6" style={{ color }} />,
    whale: <Activity className="w-6 h-6" style={{ color }} />,
    gas: <Zap className="w-6 h-6" style={{ color }} />,
    dollar: <DollarSign className="w-6 h-6" style={{ color }} />,
    droplet: <Droplet className="w-6 h-6" style={{ color }} />,
    tag: <Tag className="w-6 h-6" style={{ color }} />,
    alert: <AlertTriangle className="w-6 h-6" style={{ color }} />,
    "bar-chart": <BarChart2 className="w-6 h-6" style={{ color }} />,
  };
  return lucideMap[iconName] || <Database className="w-6 h-6" style={{ color }} />;
}
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          >
            {getChainIcon(dataset.chainIcon, dataset.chainColor)}
          </div>
          <div>
